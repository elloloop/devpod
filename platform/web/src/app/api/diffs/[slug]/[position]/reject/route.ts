...
export async function POST(
  ...
  if (!existsSync(diffsDir)) {
    return NextResponse.json({ message: "Changes requested (mock)" });
  }
  ...
  for (const file of files) {
    ...
    if (data.feature === slug && data.position === posNum) {
      ...
      if (body.comment) {
        data.reviewComment = body.comment;
      }
      writeFileSync(filePath, JSON.stringify(data, null, 2) + "\n");
      return NextResponse.json({ message: `Changes requested on D${position}` });
    }
  }
